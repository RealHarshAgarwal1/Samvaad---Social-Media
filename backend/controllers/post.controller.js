    ...
    try {
        ...
        if (postOwnerId !== likeKrneWalaUserKiId) {
            const newNotification = await Notification.create({
                ...
                type: 'like',
                post: postId,
                messageString: `${user.username} liked your post.`
            });
...
export const dislikePost = async (req, res) => {
    try {
        ...
        const postOwnerId = post.author.toString();
        if (postOwnerId !== likeKrneWalaUserKiId) {
            await Notification.deleteOne({ sender: likeKrneWalaUserKiId, recipient: postOwnerId, type: 'like', post: postId });
...
export const addComment = async (req, res) => {
    try {
        ...
        if (post.author.toString() !== commentKrneWalaUserKiId) {
            const newNotification = await Notification.create({
                recipient: post.author,
                sender: commentKrneWalaUserKiId,
                type: 'comment',
                post: postId,
                messageString: `${comment.author.username} commented: "${text.substring(0,20)}${text.length > 20 ? '...' : ''}"`
            });
